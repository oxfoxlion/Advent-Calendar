    ...
    <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl border border-white/50 shadow-lg hover:shadow-xl hover:bg-white/90 transition-all group relative h-full flex flex-col">
      ...
      {showHelp && (
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-white/95 backdrop-blur-sm rounded-2xl animate-in fade-in zoom-in-95 duration-200" onClick={() => setShowHelp(false)}>
          {/* MODIFICATION 1: New inner container with flex-col, max-h, and overflow-hidden */}
          <div 
            className="relative w-full max-w-sm bg-white p-8 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden" 
            onClick={e => e.stopPropagation()}
          >
            {/* MODIFICATION 2: Absolute positioned Close button, always visible */}
            <button 
            ...
            {/* MODIFICATION 3: Scrollable content area (excluding the main "我知道了" button) */}
            <div className="flex-1 overflow-y-auto pt-2 space-y-4 text-center">
                <div className="w-12 h-12 bg-indigo-50 text-indigo-500 rounded-full flex items-center justify-center mx-auto mb-2 shrink-0">
            ...
            {/* MODIFICATION 4: Fixed bottom button (shrink-0) */}
            <button onClick={() => setShowHelp(false)} className="px-6 py-2 bg-slate-800 text-white rounded-full text-sm font-bold hover:bg-slate-700 transition shrink-0 mt-4">
              我知道了
            </button>
          </div>
        </div>
